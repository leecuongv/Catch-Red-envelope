...
class Red_envelope:
    def __init__(self):
        #size
        random_size_value = random.uniform(RED_ENVELOPE_SIZE_RANDOMIZE[0], RED_ENVELOPE_SIZE_RANDOMIZE[1])
        size = (int(RED_ENVELOPES_SIZES[0] * random_size_value), int(RED_ENVELOPES_SIZES[1] * random_size_value))
        # moving
        moving_direction, start_pos = self.define_spawn_pos(size)
        # sprite
        self.rect = pygame.Rect(start_pos[0], start_pos[1], size[0]//1.4, size[1]//1.4)
        self.images = [image.load("Assets/red_envelope/red_envelope.png", size=size, flip=moving_direction=="right")]
        self.current_frame = 0
        self.animation_timer = 0


    def define_spawn_pos(self, size): # define the start pos and moving vel of the red_envelope
        vel = random.uniform(RED_ENVELOPES_MOVE_SPEED["min"], RED_ENVELOPES_MOVE_SPEED["max"])
    ...
    def kill(self, red_envelopes): # remove the red_envelope from the list
        red_envelopes.remove(self)
        return 1
